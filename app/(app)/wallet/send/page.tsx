...
const recentRecipients = [
  { initials: "EO", name: "Emeka Okafor", phone: "080 123 4567", trusted: true },
  { initials: "FA", name: "Fatima Aliyu", phone: "081 234 5678", trusted: true },
  { initials: "CN", name: "Chidi Nwosu", phone: "070 345 6789", trusted: false },
];
...
export default function SendMoneyPage() {
  ...
  const [recipient, setRecipient] = useState<{ name: string; phone: string; initials: string } | null>(null);
      ...
      <div style={{ background: "#FAFAF7", minHeight: "100vh" }}>
        ...
        <div className="flex items-center justify-center px-6 py-20" style={{ maxWidth: "1280px", margin: "0 auto" }}>
          <div className="card p-10 text-center max-w-sm w-full">
            ...
            <p className="text-3xl font-bold mb-2" style={{ color: "#0F6E56", letterSpacing: "-0.02em" }}>
              {amount} sent
            </p>
            <p className="text-sm text-muted mb-1">to {recipient?.name}</p>
            <p className="text-xs text-muted mb-8">Transaction processed by Squad · Ref: TXN{Math.floor(Math.random()*900000+100000)}</p>
            <div className="flex gap-3">
              <button onClick={() => router.push("/wallet")} className="btn-ghost flex-1 py-3 text-sm">Back to wallet</button>
              <button onClick={() => { setStep("recipient"); setRecipient(null); setAmount(""); setNote(""); }} className="btn-primary flex-1 py-3 text-sm">
                Send again
              </button>
            </div>
          </div>
      ...
      <div className="px-6 py-6" style={{ maxWidth: "720px", margin: "0 auto" }}>
              ...
              <div key={s} className="flex items-center flex-1">
                <div className="flex items-center gap-2">
                  <div className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0"
                    style={{ background: done ? "#0F6E56" : active ? "#0F6E56" : "#E8E6DF", color: done || active ? "#fff" : "#9E9B92" }}>
                    {done ? "✓" : i+1}
                  </div>
        ...
        {step === "recipient" && (
          <div className="flex flex-col gap-5">
            <div>
              <label className="block text-xs font-semibold text-subtle mb-2 uppercase tracking-wide">Phone or account number</label>
              <div className="flex rounded-xl overflow-hidden" style={{ border: "1px solid #E8E6DF", background: "#fff" }}>
                <span className="flex items-center px-3 text-sm border-r" style={{ background: "#F4F3EE", color: "#5C5A54", borderColor: "#E8E6DF" }}>+234</span>
                <input
                  type="tel"
                  placeholder="Enter phone number or NUBAN…"
            ...
            <div>
              <p className="text-xs font-semibold text-subtle mb-3 uppercase tracking-wide">Recent contacts</p>
              <div className="flex flex-col gap-2">
                {recentRecipients.map((r) => (
                  ...
                  >
                    ...
                    {r.trusted && <span className="badge-green">Trusted</span>}
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#D2CFC5" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round">
          ...
          <div className="flex flex-col gap-5">
            ...
            <div className="card p-4 flex items-center gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-muted">{recipient.phone}</p>
              </div>
            ...
            <div className="card p-6 flex flex-col items-center gap-4">
              ...
              <p className="text-xs text-muted">Balance: <span className="font-semibold" style={{ color: "#1C1B18" }}>₦124,500.00</span></p>
            </div>

            {/* Quick amounts */}
            <div>
              <p className="text-xs font-semibold text-subtle mb-2 uppercase tracking-wide">Quick select</p>
              <div className="flex flex-wrap gap-2">
                {quickAmounts.map((q) => (
                  <button
                    key={q}
                    onClick={() => setAmount(q.replace(/[^\d]/g, ""))}
                    className="px-4 py-2 rounded-full text-sm font-medium"
                    style={{ background: "#fff", border: "1px solid #E8E6DF", color: "#5C5A54" }}
                  >
                    {q}
                  </button>
                ))}
        ...
        {step === "confirm" && recipient && (
          <div className="flex flex-col gap-4">
            <div className="card overflow-hidden">
              ...
              {[
                { label: "To", value: `${recipient.name} · ${recipient.phone}` },
                { label: "Amount", value: `₦${parseInt(amount).toLocaleString()}`, bold: true, green: true },
                { label: "Note", value: note || "—" },
                { label: "Fee", value: "₦0.00 (free)", muted: true },
                { label: "Deducted from", value: "Squad Virtual · ₦124,500.00" },
              ].map((r, i) => (
                <div key={r.label} className="flex items-center justify-between px-5 py-3.5" style={{ borderBottom: i < 4 ? "1px solid #F4F3EE" : "none" }}>
                  <p className="text-sm text-muted">{r.label}</p>
                  <p className="text-sm font-medium" style={{ color: r.green ? "#0F6E56" : r.muted ? "#9E9B92" : "#1C1B18", fontWeight: r.bold ? 700 : 500 }}>
                    {r.value}
                  </p>
                </div>
              ))}
            ...
            <div className="rounded-xl p-4 flex items-start gap-3" style={{ background: "#E8F5F0", border: "1px solid #C3E6D8" }}>
              ...
              <p className="text-xs leading-relaxed" style={{ color: "#0F6E56" }}>
                This transfer is instant and processed securely by Squad API. You'll receive a confirmation SMS.
              </p>
            </div>
